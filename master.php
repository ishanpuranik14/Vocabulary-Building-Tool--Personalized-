...
echo '<h2 style="text-align: center;">Word : Meaning(s)</h2>';
      ...
      echo "<br />";
      echo '<h2 style="text-align: center;">Sentences</h2><br />';
      if(count($sentences) == 1){
        if(($sentence-1)>=0){
        	$sentences[$sentence] = $myArray[$sentence-1].".".$myArray[$sentence];
          //array_unshift($sentences, $myArray[$sentence-1]);
        }
        if(($sentence+1)<count($myArray)){
        	$sentences[$sentence] = $myArray[$sentence].".".$myArray[$sentence+1];
          //array_push($sentences, $myArray[$sentence+1]);
        }
      }
      foreach ($sentences as $value) {
        echo '<p> <h1>"</h1> ... '.html_entity_decode(mb_convert_encoding($value, "HTML-ENTITIES")).' ... <h1>"</h1> </p>';        
      }
